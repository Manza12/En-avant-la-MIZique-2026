...
from musictensors.model import Hit, Harmony, Chord, Rhythm, Texture, Pitch, Instrument, Orchestration, Section
...
# Orquestration
o_accompaniment = Orchestration(
    Section(Instrument('Steel Drums')),
    Section(Instrument('Steel Drums'), Instrument('Woodblock')),
    Section(Instrument('Melodic Tom'), Instrument('Steel Drums')))

s_melody = Section(Instrument('Steel Drums'))
...
antecedent_melody = t_melody_1 @ h_1 @ s_melody

### Accompaniment
h_I = octave_4 + Harmony(tonic, mediant, dominant)
h_V34 = octave_4 + Harmony(supertonic, subdominant, dominant)

antecedent_accompaniment = (t_alberti ** 4) @ (h_I + h_I + h_V34 + h_I) @ (o_accompaniment * 4)
...
consequent_melody = t_melody_2 @ h_2 @ s_melody

### Accompaniment
h_IV64 = octave_4 + Harmony(tonic, subdominant, submediant)
h_V6 = octave_4 + Harmony(leading_tone_1, supertonic, dominant)

consequent_accompaniment = (t_alberti ** 4) @ (h_IV64 + h_I + h_V6 + h_I) @ (o_accompaniment * 4)
...
# Write MIDI
midi = piece.to_midi(bpm=132, velocity=127)
